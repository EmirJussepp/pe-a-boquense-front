    ...
    <section class="page-head card">
      <div class="page-head-copy">
        ...
        <p class="page-subtitle">
          Gestión de socios inactivos y reactivación.
        </p>
      </div>
            ...
            <tr v-for="socio in rows" :key="rowId(socio)">
              ...
              <td>
                <div class="row-actions">
                  <button class="table-btn success" @click="reactivar(socio)">
                    Reactivar
                  </button>
                </div>
              </td>
...
async function loadSocios() {
  ...
  try {
    ...
    console.error("ERROR SOCIOS BAJA >>>", error)
    errorMsg.value =
...
async function reactivar(socio) {
  ...
  const ok = window.confirm(`¿Reactivar al socio ${fullName(socio)}?`)
  if (!ok) return
  ...
  try {
    ...
    console.error("ERROR REACTIVAR SOCIO >>>", error)
    window.alert(
      error?.response?.data?.message ||
      error?.response?.data?.error ||
