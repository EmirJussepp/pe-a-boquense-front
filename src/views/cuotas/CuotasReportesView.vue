...
function normalizeCobrador(item) {
  return { id: Number(item?.cobradorId ?? item?.cobradoresId ?? item?.id ?? 0), nombre: String(item?.nombre ?? "") }
}
...
const deudoresOrdenados = computed(() => {
  ...
  arr.sort((a, b) => {
    ...
    return ordenDir.value === "asc" ? (valorA < valorB ? -1 : 1) : (valorA > valorB ? -1 : 1)
  })
  return arr
})
...
const totalCuotas = computed(() => {
  ...
  return (Number(estadoCuotas.value?.pendientes ?? 0) + Number(estadoCuotas.value?.vencidas ?? 0) +
    Number(estadoCuotas.value?.pagadas ?? 0) + Number(estadoCuotas.value?.exentas ?? 0)) || 1
})
...
function limpiarFiltros() { filtro.value = { desde: null, hasta: null, cobradorId: null }; cargarTodo() }

async function cargarTodo() {
  const params = buildParams(); errores.value = []
  loadingResumen.value = true; loadingEstado.value = true; loadingDeudores.value = true; loadingCobrador.value = true
  try {
    const [resResumen, resEstado, resDeudores, resCobrador] = await Promise.allSettled([
      reportesService.obtenerResumen(params), reportesService.obtenerEstadoCuotas(params),
      reportesService.obtenerDeudaPorSocio(params), reportesService.obtenerPorCobrador(params),
    ])
    if (resResumen.status === "fulfilled") resumen.value = resResumen.value.data ?? null
    else errores.value.push("No se pudo cargar el resumen de recaudación.")
    if (resEstado.status === "fulfilled") estadoCuotas.value = resEstado.value.data ?? null
    else errores.value.push("No se pudo cargar el estado de cuotas.")
    if (resDeudores.status === "fulfilled") deudores.value = Array.isArray(resDeudores.value?.data) ? resDeudores.value.data.map(normalizeDeudor) : []
    else errores.value.push("No se pudo cargar el ranking de deudores.")
    if (resCobrador.status === "fulfilled") porCobrador.value = Array.isArray(resCobrador.value?.data) ? resCobrador.value.data.map(normalizePorCobrador) : []
    else errores.value.push("No se pudo cargar la recaudación por cobrador.")
  } finally {
    loadingResumen.value = false; loadingEstado.value = false; loadingDeudores.value = false; loadingCobrador.value = false
  }
}

function exportarExcel() {
  if (!resumen.value && !deudores.value.length && !porCobrador.value.length) { toast.warning("No hay datos para exportar."); return }
  const wb = XLSX.utils.book_new()
  if (resumen.value) {
    const resumenData = [["Métrica","Valor"],["Total Recaudado",Number(resumen.value.totalCobrado??0)],["Cantidad de Pagos",Number(resumen.value.cantidadPagos??0)],["Promedio por Pago",Number(resumen.value.promedioPorPago??0)],[],["Estado de Cuotas",""],["Pagadas",Number(estadoCuotas.value?.pagadas??0)],["Pendientes",Number(estadoCuotas.value?.pendientes??0)],["Vencidas",Number(estadoCuotas.value?.vencidas??0)],["Exentas",Number(estadoCuotas.value?.exentas??0)]]
  ...
  if (porCobrador.value.length) {
    ...
    const rows = porCobrador.value.map(item => [item.cobradorNombre,Number(item.totalCobrado),item.cantidadPagos,item.cantidadSocios])
...
async function cargarCobradores() {
  ...
  try {
    ...
    cobradores.value = Array.isArray(data) ? data.map(normalizeCobrador).filter(item => item.id > 0) : []
  } catch { cobradores.value = [] }
}

onMounted(async () => { await cargarCobradores(); await cargarTodo() })
</script>
...
.page-header {
  ...
  margin-bottom: 25px; gap: 16px; flex-wrap: wrap;
}
...
.dark-input { background: #f8fafc; border: 1px solid var(--border); border-radius: 8px; padding: 10px 12px; color: var(--text-main); font-size: 13px; min-width: 0; width: 100%; box-sizing: border-box; }
.filter-actions { display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap; }
.btn-buscar { background: var(--primary); color: white; border: none; border-radius: 8px; padding: 10px 24px; font-weight: 700; cursor: pointer; font-size: 13px; white-space: nowrap; }
.btn-limpiar { background: #f1f5f9; border: 1px solid var(--border); color: var(--text-soft); border-radius: 8px; padding: 10px 18px; font-weight: 600; cursor: pointer; font-size: 13px; white-space: nowrap; }

/* ── KPI GRID: 6 col → 3 → 2 → 2 ── */
.kpi-grid {
  display: grid;
...
.kpi-card {
  ...
  min-width: 0;        /* ← impide que el card expanda el grid */
  box-sizing: border-box;
  overflow: hidden;
}
.kpi-label { font-size: 11px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px; }
.kpi-value { font-size: 22px; font-weight: 900; color: var(--primary); line-height: 1.2; }
...
@media (max-width: 1200px) {
  ...
  .sidebar-column { order: -1; width: 100%; min-width: 0; }
}
...
@media (max-width: 768px) {
  ...
  /* 2x3 en mobile: 2 columnas, 3 filas */
...
@media (max-width: 480px) {
  /* En pantallas muy chicas seguimos con 2 col para no desperdiciar espacio */
